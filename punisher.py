'''
Agent-based model on evolution of cooperation and defection in a biological network with costly punishment. Tests the impact of diversity, costly punishment, and decreasing density of connectivity on cooperation.
'''
...
while stat <= limit:
    ...
    rest = []
    for pun in range(total_num + 1):
        if pun not in defect:
            rest.append(pun)
    punish = sample(rest, int(len(rest)/2))
            ...
            if Network[s][1] == 'D': #payoff of a defector
                for g in G.neighbors(s):
                    if Network[g][1] == 'C': #when defector meets cooperator
                        Network[s][2] += 4
                    elif Network[g][1] == 'P': #when defector meets punisher
                        Network[s][2] += -3
                    else: #payoff of defector and defector
                        Network[s][2] += 0
            elif Network[s][1] == 'P': #payoff of punisher
                for q in G.neighbors(s):
                    if Network[q][1] == 'C': #punisher meets cooperator
                        Network[s][2] += 2
                    elif Network[q][1] == 'P': #punisher meets punisher
                        Network[s][2] += -5
                    else: #pnisher meets defector
                        Network[s][2] += -2
                        
                    
            else:
                for h in G.neighbors(s): #payoff of cooperator
                    if Network[h][1] == 'C': #cooperator and cooperator
                        Network[s][2] += 2
                    elif Network[h][1] == 'P': #cooperator meets punisher
                        Network[s][2] += -5
                    else: #cooperator meets defector
                        Network[s][2] += -2
